...
const AllProducts = () => {
  ...
  const handleApprove = async (productId) => {
    try {
      await axios.post(`http://localhost:3000/admin/approve-product/${productId}`);
      fetchProducts();
    } catch (error) {
  ...
  return (
    <Tabs defaultActiveKey="waiting" id="product-tabs">
      <Tab eventKey="waiting" title="Waiting">
        <Table striped bordered hover>
          ...
          <tbody>
            {products
              .filter((product) => product.approvalStatus === 'waiting')
              .map((product) => (
                <tr key={product._id}>
                  <td>{product.productName}</td>
                  <td>
                    <Button variant="success" onClick={() => handleApprove(product._id)}>
                      Approve
                    </Button>{' '}
      ...
      <Tab eventKey="ongoing" title="Ongoing">
        <Table striped bordered hover>
          ...
          <tbody>
            {products
              .filter((product) => product.paymentStatus === 'ongoing')
              .map((product) => (
                <tr key={product._id}>
                  <td>{product.productName}</td>
                  <td>{product.paymentStatus}</td>
